...
function getAbandonedCartEmailHtml({
...
async function sendSampleEmail() {
  const targetEmail = "vision.code.vs@gmail.com";
  console.log(`Enviando correo con la Paleta Oficial Adaptada a: ${targetEmail}...`);

  const user = process.env.GMAIL_USER;
  const pass = process.env.GMAIL_PASS;

  if (!user || !pass) {
    console.error("ERROR: No se encontraron credenciales GMAIL_USER / GMAIL_PASS en .env");
    return;
  }
  ...
  const attachments = [];
  const reservaPath = path.join(__dirname, '../public/correo/reserva.png');
  const fallbackPath = path.join(__dirname, '../CORREO/reserva.png');
  const targetPath = fs.existsSync(reservaPath) ? reservaPath : fallbackPath;
  ...
  if (fs.existsSync(targetPath)) {
    ...
    console.log("✓ Imagen de reserva adjuntada:", targetPath);
  }

  const mockItems = [
    {
      title: "Retatrutide RT20",
      size: "20mg*1",
      quantity: 1,
      price: 140000,
    },
    {
      title: "Tirzepatide TZ10",
      size: "10mg*1",
      quantity: 2,
      price: 85000,
    }
  ...
  try {
    ...
    console.log("SUCCESS: Correo con Paleta Oficial enviado exitosamente!");
