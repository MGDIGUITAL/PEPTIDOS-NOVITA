...
export async function updateProduct(productId: number, formData: FormData): Promise<ActionResult> {
  try {
    ...
    if (dbError) {
      ...
      return { success: false, message: `Fallo al actualizar en la base de datos: ${dbError.message}` };
    }
    ...
    return {
      success: false,
      message: err instanceof Error ? err.message : 'Error inesperado al actualizar el producto',
    };
  }
}
